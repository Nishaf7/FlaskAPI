...
def _update_repo(repo_config, runner, tags_only):
    """Updates a repository to the tip of `main`.  If the repository cannot
    be updated because a hook that is configured does not exist in `main`,
    ...
    tag_cmd = ('git', 'describe', 'origin/main', '--tags')
    if tags_only:
        tag_cmd += ('--abbrev=0',)
    else:
        tag_cmd += ('--exact',)
    try:
        ...
        tag_cmd = ('git', 'rev-parse', 'origin/main')
        rev = cmd_output(*tag_cmd, cwd=repo_path)[1].strip()
    ...
    if hooks_missing:
        raise RepositoryCannotBeUpdatedError(
            'Cannot update because the tip of main is missing these hooks:\n'
