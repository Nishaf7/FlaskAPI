...
CNX_POOL_MAXSIZE = 32
SQL_POOL_SIZE = current_app.config.get('SQL_POOL_SIZE', 5)
...
class MySQLConnectionPool(object):
    ...
    def __init__(self, pool_size=0, pool_name=None, pool_reset_session=True,
        ...
        self._cnx_queue = queue.Queue(self._dynamic_size)
        self._check_size = None
        self._init_size = None
        self._pool_name = None
        self._increase_size = None
    ...
    def _set_check_size(self, pool_size):
        ...
        if self._dynamic_size == 0:
            calc_sizie = pool_size if pool_size else SQL_POOL_SIZE
    ...
    def add_connection(self, cnx=None):
        ...
        with CONNECTION_POOL_LOCK:
            ...
            if not cnx:
                ...
                try:
                    if self._reset_session and self._cnx_config['compress'] and cnx.get_server_version() < (5, 7, 3):
    ...
    def dynamic_load(self):
        ...
        This method increase self._pool_size + 1 connections.
        Returns None.
        """
        ctx = _ctx_stack.top
        if ctx is not None:
            if not hasattr(ctx, "pool_connections_count"):
                ctx.pool_connections_count = dict()
            if self.pool_name not in ctx.pool_connections_count.keys():
                ctx.pool_connections_count[self._pool_name] = 0

        size = self._cnx_queue.qsize()
        if size <= self._check_size:
            self._increase_size = self._pool_size + 1
            while self._pool_size < self._increase_size:
                self.add_connection()
                self._pool_size += 1
                ctx.pool_connections_count[self._pool_name] += 1
    ...
    def remove_connection(self, connection_count=0):
        """
        Close number of connections using connection_count.
        Returns int.
        """
        with CONNECTION_POOL_LOCK:
            cnt = 0
