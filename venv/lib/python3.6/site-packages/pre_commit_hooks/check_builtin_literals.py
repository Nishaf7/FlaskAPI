...
BuiltinTypeCall = collections.namedtuple(
    "BuiltinTypeCall", ["name", "line", "column"]
)
...
class BuiltinTypeVisitor(ast.NodeVisitor):
    ...
    def _check_dict_call(self, node):
        return self.allow_dict_kwargs and (
...
def check_file_for_builtin_type_constructors(
    filename, ignore=None, allow_dict_kwargs=True
):
    tree = ast.parse(open(filename, "rb").read(), filename=filename)
    visitor = BuiltinTypeVisitor(
        ignore=ignore, allow_dict_kwargs=allow_dict_kwargs
    )
    visitor.visit(tree)
...
def parse_args(argv):
    ...
    allow_dict_kwargs.add_argument(
        "--no-allow-dict-kwargs", dest="allow_dict_kwargs", action="store_false"
    )
